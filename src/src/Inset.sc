...
(class Inset of Code
	...
	(method (init theCaller theOwner theRegister)
		...
		(= oldATPs gOldATPs)
		(= oldMH gSq4MouseDownHandler)
		(= oldKH gSq4KeyDownHandler)
		(= oldDH gSq4DirectionHandler)
		(= oldWH gOldWH)
		...
		((= gSq4MouseDownHandler (EventHandler new:))
			name: {newMH}
			add: self
		)
		((= gSq4KeyDownHandler (EventHandler new:))
			name: {newKH}
			add: self
		)
		((= gSq4DirectionHandler (EventHandler new:))
			name: {newDH}
			add: self
		)
	...
	(method (dispose param1 &tmp theCaller)
		...
		(gSq4MouseDownHandler dispose:)
		(gSq4KeyDownHandler dispose:)
		(gSq4DirectionHandler dispose:)
		(gOldWH dispose:)
		...
		(= gSq4MouseDownHandler oldMH)
		(= gSq4KeyDownHandler oldKH)
		(= gSq4DirectionHandler oldDH)
		(= gOldWH oldWH)
	...
	(method (refresh)
		...
		(if (!= global36 -1)
			(DrawPic global36 dpOPEN_NO_TRANSITION dpNO_CLEAR)
		)
