...
(class IconBar of Set
	...
	(method (handleEvent pEvent &tmp temp0 pEventType temp2 temp3 theGCursorNumber theCurIcon theCurInvIcon)
		...
		(cond 
			...
			(
				...
				(= temp3
					(if
					(or (global80 canControl:) (global80 canInput:))
						(curIcon cursor?)
					else
						gWaitC
					)
			...
			((& pEventType evKEYBOARD)
				(switch (pEvent message?)
					...
					(KEY_NUMPAD0
						(if (global80 canControl:) (self swapCurIcon:))
						(pEvent claimed: 1)
					)
					(JOY_NULL
	...
	(method (dispatchEvent pEvent &tmp pEventY pEventX pEventType pEventMessage theHighlightedIcon pEventClaimed temp6 [temp7 50] temp57 theHighlightedIconSignal temp59)
		(= pEventX (pEvent x?))
		(= pEventY (pEvent y?))
		(= pEventType (pEvent type?))
			...
			(switch pEventType
				...
				(evMOUSEBUTTON
					(if
						...
						(if temp59
							(if temp57 (global1 setCursor: temp57))
							(if (& state $0800)
								(self noClickHelp:)
							else
						...
						(KEY_RETURN
							...
							(cond 
								(
									...
									(if (!= temp57 -1) (global1 setCursor: temp57))
									(if helpIconItem
										(helpIconItem signal: (| (helpIconItem signal?) $0010))
									)
								)
	...
	(method (noClickHelp &tmp temp0 temp1 temp2 temp3 gSq4WinEraseOnly)
		(= temp1 (= temp2 0))
		(= temp3 (GetPort))
		(= gSq4WinEraseOnly (gSq4Win eraseOnly?))
		(gSq4Win eraseOnly: 1)
		(while
		(not ((= temp0 ((global80 curEvent?) new:)) type?))
			...
			(cond 
				...
				(gTheNewSq4Dialog (gTheNewSq4Dialog dispose:))
				(else (= temp1 0))
			)
			(temp0 dispose:)
		)
		(gSq4Win eraseOnly: gSq4WinEraseOnly)
		(global1 setCursor: 999 1)
		(if gTheNewSq4Dialog (gTheNewSq4Dialog dispose:))
		(SetPort temp3)
