...
(class Cycle of Obj
	...
	(method (nextCel)
		(return
			(if
			(< (Abs (- gTixCnt cycleCnt)) (client cycleSpeed?))
				(client cel?)
			else
				(= cycleCnt gTixCnt)
				(+ (client cel?) cycleDir)
...
(class CT of Cycle
	...
	(method (doit &tmp cTNextCel clientLastCel)
		...
		(if
		(and (== gTixCnt cycleCnt) (== endCel (client cel?)))
			(self cycleDone:)
		)
	)
...
(class SyncWalk of Fwd
	...
	(method (nextCel)
		(= cycleCnt (+ gTixCnt (client cycleSpeed?)))
		(super nextCel:)
	)
)
...
(class Motion of Obj
	...
	(method (init theClient theX theY theCaller &tmp [temp0 2] clientX clientCycler)
		...
		(= b-moveCnt (+ 1 (client moveSpeed?) gTixCnt))
	...
	(method (doit &tmp [temp0 6])
		(if
		(>= (Abs (- gTixCnt b-moveCnt)) (client moveSpeed?))
			(= b-moveCnt gTixCnt)
			(DoBresen self)
		)
	)
